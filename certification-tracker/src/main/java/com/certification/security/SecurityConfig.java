...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.cors(Customizer.withDefaults()).csrf(csrf -> csrf.disable()).authorizeHttpRequests(auth -> auth
				...
				// Public images (img tags don't send Authorization headers)
				.requestMatchers(HttpMethod.GET, "/api/images/**").permitAll()

				// Auth endpoints
				.requestMatchers("/api/auth/register", "/api/auth/login").permitAll()

				// Catalog: GET allowed to all authenticated roles, mutations restricted to
				// ADMIN
				...
				// Goals (Week 2): must be authenticated (method-level @PreAuthorize handles
				// finer rules)
				.requestMatchers("/api/goals/**").authenticated()

				// Users (if any): authenticated
